...
import { FiCheck, FiTrash2, FiClock, FiUser, FiMail, FiFileText, FiChevronDown } from 'react-icons/fi';
...
export default function AdminComments() {
    ...
    const handleCommentAction = async (action, commentId, replyId = null) => {
        try {
            setError('');
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || `Failed to ${action} comment`);
            }

            // Refetch comments after successful action
            await fetchComments();
        } catch (err) {
            setError(err.message);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="bg-white rounded-lg shadow overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200">
                    <h2 className="text-2xl font-bold text-gray-800">Comments Management</h2>
                    <p className="mt-1 text-sm text-gray-600">
                        Manage all user comments and replies from this dashboard
                    </p>
                </div>
                                                            ...
                                                            <div className="ml-3 flex-1">
                                                                ...
                                                                <div className="mt-3 flex justify-end space-x-2">
                                                                    ...
                                                                    {activeTab === 'pending' && !reply.isApproved && (
                                                                        <button
                                                                            onClick={() => handleCommentAction('approve', comment._id, reply._id)}
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        className="inline-flex items-center px-2.5 py-1 border border-transparent text-xs font-medium rounded shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                                                                    >
                                                                        <FiTrash2 className="mr-1" /> Delete
                                                                    </button>
                                                                </div>
