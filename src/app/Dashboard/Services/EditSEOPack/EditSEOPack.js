...
const EditSEOPackage = ({ params }) => {
    ...
    useEffect(() => {
        if (!id) {
            ...
            router.push('/admin-dashboard/services/seo-packages/all');
            return;
        }

        const fetchPackage = async () => {
            try {
                const res = await fetch(`/api/services/seopack?id=${id}`, { cache: 'no-store' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch('/api/services/seopack', {
                method: 'PUT',
                body: form,
            });
