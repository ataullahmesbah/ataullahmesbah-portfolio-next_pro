        ...
        const fetchReviews = async () => {
            try {
                ...
                if (totalReviewsCount > 0) {
                    ...
                    setAverageRating(totalRatingSum / totalReviewsCount);
                }
                        ...
                        <SwiperSlide key={review._id}>
                            <div className="bg-slate-700 text-gray-200 p-6 rounded-xl mb-6 max-w-sm mx-auto">
                                <div className="flex justify-center items-center mb-4 relative">
                                    {review.image ? (
                                        <Image src={review.image} alt={review.user_name} width={50} height={50} className="rounded-full w-14 h-14 border border-gray-400" />
                                    ) : (
                                ...
                                <Rating initialRating={review.rating} readonly emptySymbol={<FaRegStar className="text-yellow-500" />} fullSymbol={<FaStar className="text-yellow-500" />} fractions={2} />
                                <p className="text-sm mt-4">{review.description.slice(0, 30)}...</p>
                            </div>
                        </SwiperSlide>
                    ))}
                </Swiper>
