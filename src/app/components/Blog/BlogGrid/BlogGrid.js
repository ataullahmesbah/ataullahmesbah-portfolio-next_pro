...
export default function BlogGrid({ blogs, currentPage, totalPages, error }) {
    if (error) {
        return (
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-6 text-center" data-aos="fade-up">
                <div className="text-red-400 text-4xl lg:text-6xl mb-4">⚠️</div>
                <h3 className="text-lg lg:text-xl font-semibold text-red-400 mb-2">Failed to Load Blogs</h3>
                <p className="text-gray-400 text-sm lg:text-base mb-4">{error}</p>
                <button
                    onClick={() => window.location.reload()}
                    className="bg-red-600 text-white px-4 lg:px-6 py-2 rounded-lg hover:bg-red-700 transition-colors text-sm lg:text-base"
                >
                    Try Again
                </button>
            </div>
